...
resource "aws_iam_policy" "ebs_csi_iam_policy" {
  name        = "${var.customer}-${var.environment}-AmazonEKS_EBS_CSI_Driver_Policy"
  path        = "/"
...
resource "aws_iam_role" "ebs_csi_iam_role" {
  name = "${var.customer}-${var.environment}-ebs-csi-iam-role"
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {            
            "${aws_iam_openid_connect_provider.oidc_provider.arn}:sub": "system:serviceaccount:kube-system:ebs-csi-controller-sa"
          }
        }        
  ...
  tags = {
    tag-key = "${var.customer}-${var.environment}-ebs-csi-iam-role"
  }
}
