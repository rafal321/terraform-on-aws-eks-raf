...
resource "aws_iam_policy" "externaldns_iam_policy" {
  name        = "${var.cluster_name}-AllowExternalDNSUpdatesPolicy"
  path        = "/"
  description = "EKS External DNS IAM Policy"
  policy = jsonencode({
    "Version" : "2012-10-17",
    "Statement" : [
      {
...
resource "aws_iam_role" "externaldns_iam_role" {
  name        = "${var.cluster_name}-AllowExternalDNSUpdatesRole"
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = { # RAF: this creates service account - Trust Relationship
            ...
            "${local.aws_iam_oidc_connect_provider_extract_from_arn}:sub" : "system:serviceaccount:kube-system:external-dns"
            # NAMESPACE!
          }
        }
      },
  ...
  tags = {
    tag-key              = "${var.cluster_name}-AllowExternalDNSUpdates"
