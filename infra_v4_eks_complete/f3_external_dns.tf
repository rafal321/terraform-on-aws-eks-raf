...
resource "helm_release" "external_dns" {
  ...
  set {
    ...
    value = "registry.k8s.io/external-dns/external-dns" # dafault anyway
  }
  set {
    name  = "serviceAccount.create" # default anyway
    value = "true"
  }
  set {
  ...
  set { # Default is aws (https://github.com/kubernetes-sigs/external-dns/tree/master/charts/external-dns)
    name  = "provider"
    value = "aws"
  }
  set {
    ...
    value = "sync"   # "sync" will ensure that when ingress resource is deleted, equivalent DNS record in Route53 will get deleted [`sync` & `upsert-only`]
  }
}
...
# 223. Step-02: Review ExternalDNS TFConfigs
